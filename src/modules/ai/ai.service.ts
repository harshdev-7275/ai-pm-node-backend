...
import type { ChatRequest, ChatIdentity, ChatResponse, SuggestionRequest, SuggestionsResponse } from './ai.types.js'
import { logger } from '../../utils/logger.js'
import { recordSyncStart, recordSyncFinish, type SyncStartInput } from './kgSyncLog.service.js'
import { getSuggestions as buildSuggestions } from './ai.suggestions.js'
...
/**
 * Return contextual prompt suggestions (template engine, <5 ms, zero LLM cost).
 * Conversation-aware follow-ups are now generated by the PM agent and delivered
 * directly through the SSE stream as a `suggestions` event — no extra call needed.
 */
export const getAiSuggestions = async (
  input:  SuggestionRequest,
  orgId:  string,
): Promise<SuggestionsResponse> => {
  const suggestions = await buildSuggestions(input, orgId)
  return { suggestions }
}
